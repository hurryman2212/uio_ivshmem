...
static int ivshmem_pci_probe(struct pci_dev *dev,
  ...
  /* Allocate upto MSI-X limitation, which is 2048. */
  ret = pci_alloc_irq_vectors(dev, 1, 2048, PCI_IRQ_LEGACY | PCI_IRQ_MSIX);
  if (ret < 0)
    goto out_unmap2;
  if (ret == 0)
    goto out_vector;

  /* UIO allows only one IRQ to be used... */
  ...
  info->version = __TARGET_VERSION__;
